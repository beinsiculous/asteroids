...
This is a single-crate game (`insiculous_asteroids`) built on the in-house `insiculous_2d` ECS engine. `AsteroidsGame` (in `src/types.rs`) implements the engine's `Game` trait in `src/main.rs` — `init()` loads the font and spawns only the background + deforming grid (ships and rocks spawn fresh in `start_game()` because ship count depends on the picked mode); achievements register in `register_achievements()`, which the engine calls before the window opens, and `cargo run -- --achievements-manifest <path>` exports the list. With `--features editor` the identical game runs inside the engine's scene editor via `editor_integration::run_game_with_editor`; no game code changes between the two modes.
