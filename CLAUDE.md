...
This is a single-crate game (`insiculous_asteroids`) built on the in-house `insiculous_2d` ECS engine. `AsteroidsGame` (in `src/types.rs`) implements the engine's `Game` trait in `src/main.rs` — `init()` loads the font and spawns only the background + deforming grid (ships and rocks spawn fresh in `start_game()` because ship count depends on the picked mode); achievements register in `register_achievements()`, which the engine calls before the window opens, and `cargo run -- --achievements-manifest <path>` exports the list. With `--features editor` the identical game runs inside the engine's scene editor via `editor_integration::run_game_with_editor`, and at `/playground/asteroids/` in the browser (the same feature, built by the engine's `build_wasm.sh --kind editor` and served from the site); no game code changes between the modes.
...
**The game steps physics itself.** `AsteroidsGame` owns a `PhysicsSystem` (`PhysicsConfig::space()` — zero gravity) and calls `self.physics.update(ctx.world, ctx.delta_time)` inside `update_gameplay()`, followed by a single `take_collision_events()` drain shared by all consumers; stray bullets expire in the engine's frame tail (`ecs::LifetimeSystem`), which runs after the game's update — the game owns no lifetime system. Body recipes (spawning.rs):
...
**Achievements** (11, defined in `achievements.rs`, registered in `register_achievements()`, which the engine calls before the window opens; `cargo run -- --achievements-manifest <path>` exports the list): wave-5 per mode ×4, score tiers 10k/30k, and skill (Sharpshooter 10-hit streak, Close Call ≤60 px, Untouchable 3 deathless waves, Double Tap 2 kills ≤0.5 s, Living on the Edge = wave 5 on the last total life). `DISPLAY_SECTIONS` drives the achievements page; a test enforces it covers every registered id. Extra life every 10,000 points, per ship.
...
- Asteroids become **roided-out flexing meatballs** in the same 3 size classes; splitting a big one is a flex-burst into smaller, angrier meatballs. The **angry meatball** is a recurring cross-game character (hazard in Snake's "Bratdog").
